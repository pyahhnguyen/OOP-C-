...
int main(){
    Sinhvien x;
    cin>>x.msv;
    getline(cin, x.ten);// hàm nhap chuoi
    cin.ignore();//Việc sử dụng hàm getline có thể gây mất biến khi nhấn enter nghĩa là nếu bạn tạo 1 biến có giá trị số sau đó bạn getline và in 1 chuỗi ký tự.
    cin>>x.gpa;
    cin.ignore();
    getline(cin, x.ten);
cout<<x.msv<<" "<<x.ten<<""<<fixed<< setprecision(2)<< x.gpa<<" "<<x.dc<<endl;reutrn
return 0;
}
