...
struct node
{
	sinhvien sv20139072; // d? li?u c?a node ==> d? li?u mà node s? luu tr?
...
void ThemNodeVaoCay(TREE &t, sinhvien x)
{
	...
	if (t == NULL)
	{
		...
		p->sv20139072 = x;// thêm d? li?u x vào data
		p->pLeft = NULL;
		p->pRight = NULL;
	...
	else // cây có t?n t?i ph?n t?
 	{
		...
		else if (compare(t->sv20139072 ,x) < 0 ) // n?u ph?n t? thêm vào mà l?n hon node g?c ==> thêm nó vào bên ph?i
		{
